...
def calc_vmaf(source_video, output_video, work_dir, need_deinterlace, subsample, pool = "mean"):
    ret = {}
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print "%s source_file output_video n_split [seg_dur in seconds, default 4.0]" % sys.argv[0]
        exit(0)

    source_video = sys.argv[1]
    output_video = sys.argv[2]
    n_split = int(sys.argv[3])
    seg_dur = float(sys.argv[4]) if len(sys.argv) >= 5 else 4.0

    work_dir = os.path.dirname(output_video) or "."
    cur_start, total_dur, video_fps = get_video_info(source_video)
    split_dur = int((total_dur - cur_start) / n_split / seg_dur + 1) * seg_dur
    ...
    while cur_start < total_dur:
        cur_dur = min(split_dur, total_dur - cur_start)
        seg_file = "%s/%d_%d.webm" % (shot_tmp_dir, idx, int(cur_start * video_fps))
        num_frame = round(cur_dur * video_fps)
        gop = seg_dur * video_fps
    ...
    print msg
    vmaf_ret = calc_vmaf(source_video, output_video, work_dir, True, int(video_fps))
    if DELETE_TEMP:
